...
UBUNTU_TRANSLATION_DOMAIN="svggraph.liu-xiao-guo"
...
aptest.commands = QML2_IMPORT_PATH=$$OUT_PWD/backend bash $$PWD/svggraph/tests/autopilot/run
aptest.depends  = sub-svggraph sub-backend-App

unittest.target   = check
unittest.commands = /usr/bin/qmltestrunner -input $$PWD/backend/tests/unit -import $$OUT_PWD/backend
unittest.commands += && /usr/bin/qmltestrunner -input $$PWD/svggraph/tests/unit -import $$OUT_PWD/backend
unittest.depends  = sub-svggraph sub-backend-App
